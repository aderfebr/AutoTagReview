...
from transformers import get_scheduler, logging, BertTokenizer, BertModel
...
def save_model(config, epoch, bert, classify_net, tokenizer, save_path=None):
    if save_path is None:
        save_path = f"./checkpoint/{config['dataset']}/{epoch+1}"
    
    os.makedirs(save_path, exist_ok=True)
    
    bert.save_pretrained(save_path)
    torch.save(classify_net.state_dict(), f"{save_path}/student_model_file.bin")
...
if __name__ == '__main__':
    ...
    for epoch in range(config['epochs']):
        ...
        save_model(config, epoch, bert, classify_net, tokenizer)
        with torch.no_grad():
            bert.eval()
            classify_net.eval()
            accuracy.reset_acc()
